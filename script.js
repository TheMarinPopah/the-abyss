window.addEventListener('DOMContentLoaded', () => {
    ...
    // Player Colors
    const playerColors = [
        0xF4D03F, 0xE74C3C, 0x2ECC71, 0x3498DB, 
        0xE67E22, 0x9B59B6, 0x1ABC9C, 0xE91E63
    ];
    ...
    function createNoiseTexture(baseColor, noiseAmount, size = 64) {
        ...
        for (let i = 0; i < imageData.data.length; i += 4) {
            ...
            imageData.data[i] += noise; imageData.data[i+1] += noise; imageData.data[i+2] += noise;
        }
    ...
    function generateMaze(width, height) {
        const grid = [];
        for (let y = 0; y < height; y++) { grid[y] = []; for (let x = 0; x < width; x++) grid[y][x] = 1; }
        function carve(x, y) {
            grid[y][x] = 0;
        ...
        carve(1, 1);
        for (let y = 0; y < height; y++) { for (let x = 0; x < width; x++) { if (y === 0 || y === height - 1 || x === 0 || x === width - 1) grid[y][x] = 1; } }
        return grid;
    }
    ...
    function initNetworking(hosting) {
        ...
        const peerOptions = { debug: 2, secure: true, serialization: 'json', config: { iceServers: [ { urls: 'stun:stun.l.google.com:19302' } ] } };
        if (hosting) {
            const roomCode = generateRoomCode();
            peer = new Peer(roomCode, peerOptions);
            isHost = true; myColorIndex = 0;
            peer.on('open', (id) => { myId = id; lobbyCodeDisplay.textContent = id; showScreen('host'); maze = generateMaze(state.mazeSize, state.mazeSize); });
            peer.on('error', (err) => { console.error(err); alert('Error: ' + err.type); showScreen('menu'); });
            peer.on('connection', (conn) => { if (connections.length >= 7) return; setupConnection(conn); });
        }
    }
    ...
    function setupConnection(conn) {
        conn.on('open', () => {
            if (isHost) {
                ...
                for (let id in otherPlayers) currentPlayers[id] = otherPlayers[id];
            ...
            } else { myConn = conn; showScreen('waiting'); }
        });
        ...
        conn.on('error', (err) => { if (!isHost) { joinError.textContent = "Connection failed."; joinError.style.display = 'block'; showScreen('join'); } });
    }

    function handleData(data, senderId) {
        if (data.type === 'init') { maze = data.data; myColorIndex = data.yourColorIndex; for (let id in data.players) updateOtherPlayer(id, data.players[id].x, data.players[id].z, data.players[id].angle, data.players[id].colorIndex); }
        else if (data.type === 'new-player') { if (!otherPlayers[senderId]) otherPlayers[senderId] = { colorIndex: data.colorIndex }; }
        else if (data.type === 'player-left') { removeOtherPlayer(data.id); }
        else if (data.type === 'start') { startGame(); }
        else if (data.type === 'move') { updateOtherPlayer(data.id, data.x, data.z, data.angle, data.colorIndex); if (isHost) broadcastData(data, senderId); }
    ...
    function updateOtherPlayer(id, x, z, angle, colorIndex) {
        if (id === myId) return;
        if (!otherPlayers[id]) { const cIdx = (colorIndex !== undefined) ? colorIndex : 1; const mesh = createCharacterMesh(playerColors[cIdx] || playerColors[1]); scene.add(mesh); otherPlayers[id] = { mesh, x, z, angle, colorIndex: cIdx }; }
        otherPlayers[id].mesh.position.set(x, 0, z); otherPlayers[id].mesh.rotation.y = angle; otherPlayers[id].x = x; otherPlayers[id].z = z; otherPlayers[id].angle = angle;
    }
    ...
    // --- Three.js Setup ---
    function initThree() {
        scene = new THREE.Scene(); scene.background = new THREE.Color(0x807040); scene.fog = new THREE.Fog(0x807040, 1, 25);
        camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 100); camera.position.y = 1.5;
        renderer = new THREE.WebGLRenderer({ antialias: true }); renderer.setSize(window.innerWidth, window.innerHeight); renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2)); gameContainer.appendChild(renderer.domElement);
        clock = new THREE.Clock();
        const ambientLight = new THREE.AmbientLight(0x908060, 0.6); scene.add(ambientLight);
        playerLight = new THREE.PointLight(0xffffff, 0.8, 20); playerLight.castShadow = true; scene.add(playerLight);
        
        createFloor();
        if (maze.length === 0) maze = generateMaze(state.mazeSize, state.mazeSize);
        createWalls();
        
        playerMesh = createCharacterMesh(playerColors[myColorIndex]); scene.add(playerMesh);
        player.x = 1.5 * state.cellSize; player.z = 1.5 * state.cellSize; player.angle = 0;
        
        // Reset Stamina
        staminaBarUI = document.getElementById('stamina-bar');
    ...
    function createFloor() {
        const carpetTexture = createNoiseTexture('#6B5344', 30, 128); carpetTexture.repeat.set(20, 20);
        ...
        const ceilTexture = createNoiseTexture('#E0D8C0', 10, 128); ceilTexture.repeat.set(20, 20);
        const ceiling = new THREE.Mesh(new THREE.PlaneGeometry(200, 200), new THREE.MeshStandardMaterial({ map: ceilTexture, side: THREE.BackSide })); ceiling.rotation.x = Math.PI / 2; ceiling.position.y = state.wallHeight; scene.add(ceiling);
    }
    ...
    function createWalls() {
        ...
        for (let y = 0; y < state.mazeSize; y++) {
            for (let x = 0; x < state.mazeSize; x++) {
                if (maze[y][x] === 1) {
                    const wall = new THREE.Mesh(new THREE.BoxGeometry(state.cellSize, state.wallHeight, state.cellSize), wallMat);
    ...
    function updateCamera() {
        camera.position.x = player.x; camera.position.z = player.z; camera.rotation.y = player.angle;
    ...
    function checkCollision(newX, newZ) {
        const margin = 0.4;
        const gx = Math.floor(newX / state.cellSize); const gz = Math.floor(newZ / state.cellSize);
    ...
    function movePlayer(delta) {
        ...
        // Update UI
        if (staminaBarUI) {
            const percent = (state.stamina / state.maxStamina) * 100;
            staminaBarUI.style.width = percent + '%';
        }
        ...
        if (isHost) { broadcastData({ type: 'move', id: myId, x: player.x, z: player.z, angle: player.angle, colorIndex: myColorIndex }); } 
        else if (myConn && myConn.open) { sendData({ type: 'move', id: myId, x: player.x, z: player.z, angle: player.angle }); }
    }
    ...
    function animate() {
        ...
        if (state.isPlaying) { movePlayer(delta); playerLight.intensity = 0.8 + Math.sin(time * 10) * 0.1; if (playerMesh) playerMesh.position.y = Math.sin(time * 3) * 0.03; }
        renderer.render(scene, camera);
    }

    function showScreen(name) {
        menuScreen.classList.remove('active'); guideScreen.classList.remove('active'); gameScreen.classList.remove('active'); winScreen.classList.remove('active'); joinScreen.classList.remove('active'); hostScreen.classList.remove('active'); waitingScreen.classList.remove('active');
        if (name === 'menu') menuScreen.classList.add('active'); if (name === 'guide') guideScreen.classList.add('active'); if (name === 'game') gameScreen.classList.add('active');
        if (name === 'win') winScreen.classList.add('active'); if (name === 'join') joinScreen.classList.add('active'); if (name === 'host') hostScreen.classList.add('active'); if (name === 'waiting') waitingScreen.classList.add('active');
    }

    function startGame() {
        state.isPlaying = true; state.distance = 0; distanceCounter.textContent = '0';
        if (renderer) { gameContainer.removeChild(renderer.domElement); renderer.dispose(); }
        showScreen('game'); initThree();
    }
    ...
    joinBtn.addEventListener('click', () => {
        ...
        peer.on('open', (id) => { myId = id; const conn = peer.connect(hostCode); myConn = conn; conn.on('error', () => { joinError.textContent = "CONNECTION FAILED"; joinError.style.display = 'block'; showScreen('join'); }); setupConnection(conn); });
        peer.on('error', (err) => { if (err.type === 'peer-unavailable') joinError.textContent = "INVALID CODE"; else joinError.textContent = "ERROR: " + err.type; joinError.style.display = 'block'; showScreen('join'); });
    });
